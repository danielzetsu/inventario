...
<script src="{{ URL::to('/SmartAdmin/') }}/js/plugin/bootstrap-wizard/jquery.bootstrap.wizard.min.js"></script>
<script src="{{ URL::to('/SmartAdmin/') }}/js/plugin/fuelux/wizard/wizard.min.js"></script>
...
<script src="{{ URL::to('/SmartAdmin/') }}/js/plugin/summernote/summernote.min.js"></script>

<script src="{{url('/')}}/table-to-json/lib/jquery.tabletojson.js"></script>

<!--<script src="{{ url('/') }}/js/datatables.sum().js"></script>-->

<!-- pusher version 4.0 -->
<!--<script src="{{ url('/') }}/js/wspusher.js"></script>-->
<!--<script type="text/javascript">
	 	 console.log("pusherapp");
...
jQuery('#dt_basic').dataTable({
	"sDom": "<'dt-toolbar'<'col-xs-12 col-sm-4'f><'col-sm-6 col-xs-12 hidden-xs'l>r>"+
		"t"+
	...
	"oLanguage": {
      "sSearch": "Buscar:",
      "sZeroRecords": "No se encontraron registros que coincidan con su busqueda",
      "sEmptyTable": "No datos encontrados",
...
var responsiveHelper_datatable_col_reorder_2 = undefined;
var responsiveHelper_datatable_tabletools_2 = undefined;

jQuery('#dt_basic_2').dataTable({
	...
	"preDrawCallback" : function() {
		...
		if (!responsiveHelper_dt_basic_2) {
			responsiveHelper_dt_basic_2 = new ResponsiveDatatablesHelper(jQuery('#dt_basic_2'), breakpointDefinition);
		}
	},
	"rowCallback" : function(nRow) {
...
var responsiveHelper_datatable_col_reorder_3 = undefined;
var responsiveHelper_datatable_tabletools_3 = undefined;

jQuery('#dt_basic_3').dataTable({
	"sDom": "<'dt-toolbar'<'col-xs-12 col-sm-6'f><'col-sm-6 col-xs-12 hidden-xs'l>r>"+
		"t"+
		"<'dt-toolbar-footer'<'col-sm-6 col-xs-12 hidden-xs'i><'col-xs-12 col-sm-6'p>>",
	"autoWidth" : true,
	...
	"preDrawCallback" : function() {
		...
		if (!responsiveHelper_dt_basic_3) {
			responsiveHelper_dt_basic_3 = new ResponsiveDatatablesHelper(jQuery('#dt_basic_3'), breakpointDefinition);
		}
	},
	"rowCallback" : function(nRow) {
...
var responsiveHelper_datatable_fixed_column_4 = undefined;
var responsiveHelper_datatable_col_reorder_4 = undefined;
var responsiveHelper_datatable_tabletools_4 = undefined;

jQuery('#dt_basic_4').dataTable({
	"sDom": "<'dt-toolbar'<'col-xs-12 col-sm-6'f><'col-sm-6 col-xs-12 hidden-xs'l>r>"+
		"t"+
		"<'dt-toolbar-footer'<'col-sm-6 col-xs-12 hidden-xs'i><'col-xs-12 col-sm-6'p>>",
	"autoWidth" : true,
	...
	"preDrawCallback" : function() {
		...
		if (!responsiveHelper_dt_basic_4) {
			responsiveHelper_dt_basic_4 = new ResponsiveDatatablesHelper(jQuery('#dt_basic_4'), breakpointDefinition);
		}
	},
	"rowCallback" : function(nRow) {
  ...
  jQuery("#crear_punto_venta").remove();
  jQuery("#crear_empleados").remove();
...
function buscar_dialogo(controlador,div_id,opc,valor){
	...
	jQuery.ajax({
		...
		,success: function(data){
			jQuery("#body_modal_"+div_id).html(data);
			jQuery("#MyModal"+div_id).modal("show");
		}
		,error: function(data){
			jQuery("#body_modal_"+div_id).html("Error: "+ data);
			jQuery("#MyModal"+div_id).modal("show");
		}	
	});		
}
...
function validar(controlador,opcion,valor){
	...
	jQuery.ajax({
		type:"GET"
		,url:"{{url('/')}}/"+controlador+"/validar/"+valor+"/"+opcion
		,async:true
		,success: function(data){
...
/*Fin Funcion autocomplete de clientes */
	...
	if( jQuery( "#fecha"      ) ){ jQuery( "#fecha"      ).datepicker({ dateFormat: "dd/mm/yy"}); 	}
	if( jQuery( "#fecha_nac"      ) ){ jQuery( "#fecha_nac"      ).datepicker({ dateFormat: "dd/mm/yy"}); 	}
			
	if( jQuery( "#vencimiento"      ) ){ jQuery( "#vencimiento"      ).datepicker({ dateFormat: "dd/mm/yy"}); 	}
	

	if( jQuery( "#desde"        ) ){ jQuery( "#desde"        ).datepicker({ dateFormat: "dd/mm/yy"}); 	}
	if( jQuery( "#pronto_pago"  ) ){ jQuery( "#pronto_pago"  ).datepicker({ dateFormat: "dd/mm/yy"}); 	}
	if( jQuery( "#hasta"        ) ){ jQuery( "#hasta"        ).datepicker({ dateFormat: "dd/mm/yy"}); 	}
